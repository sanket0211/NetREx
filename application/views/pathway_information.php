...
<nav>
        <ul>
            <li id="na" data-step="12" data-intro="Click here to view the node-attributes table which show fc and p-value information along with kegg-pathway information, ic4r(homology information, rna seq experiments), gene information provided by rap db, gene belonging to module, gene description and GO term of the input genes." onclick="fetched_pathways_pressed()"  class="menu-button selected">Fetched Pathways</li>
            <li id="ea" data-step="13" data-intro="Click here to see the tabluted information about the links in the network seen above. " onclick="pathway_mapping_pressed()" class="menu-button selected">Pathway Mapping</li>
...
<div id="pm">
  <table class="container" id ='tblCustomers1'>
  ...
  foreach($path_gene_info as $gene){
    $cnt=$cnt+1;
    echo '<tr>';
    echo '<td>'.$cnt.'</td>';
    echo '<td>'.$gene->cnt.'</td>';
    
    echo '<td>'.$gene->pathway_name.'</td>';
    
    echo '<td>'.$gene->os_id.'</td>';
    echo '<form action="http://bioinf.iiit.ac.in/ricenet/ricenetwork/index.php/home/pathway/'.$gene->os_id2.'/'.$gene->id.'">';
    echo '<td><input type="submit" value="View Pathway" />';
    echo '</form>';
    echo '</tr>';
